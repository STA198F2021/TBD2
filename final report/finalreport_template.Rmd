...
```{r seasons comparex, echo=FALSE, warning=FALSE}
#data/libraries
library(dplyr)
library(tidyverse)
library(viridis)
...
```{r seasons compare, echo=FALSE, warning=FALSE}
...
This Stringency index was calculated on a daily basis. The weekly average indices were used to merge this data with the influenza data.

In order to visualize and better understand the relationship between the stringency index and influenza infections, we selected the five US states with the most COVID-19 cases recorded.
...
merged_5_states%>%
ggplot()+
geom_smooth(aes(x = WEEK, y = Stringency_Index, color = as.factor(YEAR)), se=FALSE)+
  facet_wrap(~STATE)+
...
<<<<<<< HEAD
```{r linear model stringency states chunk 2}
merged_5_states%>%
  filter(YEAR == 2020)%>%
  ggplot(aes(`Stringency_Index`, `NUM INFLUENZA DEATHS`))+
  geom_smooth(method = "lm")+
