...
covid_flu_table <- flu_table %>%
  ...
  group_by(seasonweek)
covid_flu_table %>%
  ggplot( aes(x = seasonweek)) +
  geom_line(aes(y = total_flu_deaths, group = season)) +
  geom_smooth(aes(y = total_flu_deaths))
```

```{r model}
nest_covid_flu_table <- covid_flu_table%>%
  nest()
flu_death_fit <- function(nest_covid_flu_table) {lm(total_flu_deaths ~ covidstatus, data = nest_covid_flu_table)}
flu_hosp_fit <- function(nest_covid_flu_table) {lm(total_flu_patients ~ covidstatus, data = nest_covid_flu_table)}
nest_covid_flu_table <- nest_covid_flu_table%>%
  mutate(deathmodel = map(data, flu_death_fit), hospmodel = map(data, flu_hosp_fit))
```
Based on the model, there is 
